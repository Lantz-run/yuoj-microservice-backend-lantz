...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    public long userRegister(String userAccount, String userPassword, String userName, String checkPassword, String userEmail) {
        // 1. 校验
        if (StringUtils.isAnyBlank(userAccount, userPassword, userName, checkPassword, userEmail)) {
        ...
        if (userEmail.length() < 15) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户邮箱过短");
        }
        if (userName.isEmpty()) {
    ...
    @Override
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request, String userEmail) {
        // 1. 校验
        ...
        if (userEmail.length() < 15 || userEmail.length() > 20) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户邮箱错误");
        }
        // 2. 加密
        String encryptPassword = DigestUtils.md5DigestAsHex((SALT + userPassword).getBytes());
        // 查询用户是否存在
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        // 从请求属性中获取用户信息（由 JWT 拦截器设置）
//        User currentUser = (User) request.getAttribute("loginUser");
